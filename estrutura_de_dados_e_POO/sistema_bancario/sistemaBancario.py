...
def log_transacao(func):
    def envelope(*args, **kwargs):
        ...
        with open(ROOT_PATH / "log.txt", "a") as arquivo:
            arquivo.write(
                f"[{data_hora}] | Função '{func.__name__}' executada com argumentos: {args}, {kwargs}.\n"
            )
...
def criar_cliente(clientes):
    ...
    data_hora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(ROOT_PATH / "lista_clientes.txt", "a") as arquivo:
            arquivo.write(
                f"[{data_hora}] | Cliente: {cliente.nome} | CPF: {cliente.cpf}\n"
            )
...
def criar_conta(numero_conta, clientes, contas):
    ...
    data_hora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(ROOT_PATH / "lista_contas.txt", "a") as arquivo:
            arquivo.write(
                f"[{data_hora}] | Conta: {conta.numero} | Cliente: {conta.cliente}\n"
            )
